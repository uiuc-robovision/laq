...
class GibsonHouse:
    ...
    def semantics(self):
        if self._semantics is None:
            if self.data['split_tiny'] != 'none':
                folder = '/data01/arjung2/gibson_tiny_annotations/verified_graph'
            else:
                folder = '/data01/arjung2/3DSceneGraph_medium'
    ...
    def get_env(self, **kwargs):
        return HabitatTestEnv(f'/data01/arjung2/gibson/{self.name}.glb',
                              **kwargs)


def get_houses(split=['train', 'val']):
    with open('/data01/arjung2/gibson/metadata.json') as json_file:
